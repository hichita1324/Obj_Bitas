...
//CreateTimes : 06/04/2016 08:28:39.AM

namespace Bitas
{
	public class KeysList_DM_ChietKhau
	{
		//ListAll
		public List<Ob_DM_ChietKhau> _ListAll(SqlConnection conn)
		{
			...
			while (reader.Read())
			{
				...
				if(reader[0] != DBNull.Value){ ob.ID_CHIETKHAU = Convert.ToInt32(reader[0]);}
				if(reader[1] != DBNull.Value){ ob.MACHIETKHAU = Convert.ToString(reader[1]);}
				if(reader[2] != DBNull.Value){ ob.TENCHIETKHAU = Convert.ToString(reader[2]);}
				list.Add(ob);
			}
			command.Dispose();
			command = null;
		...
		//ListAll
		public List<Ob_DM_ChietKhau> _ListAll_sp(string StoredName, SqlConnection conn)
		{
			if (conn.ConnectionString == "")
			{
				return null;
		...
		//ListWhere
		public List<Ob_DM_ChietKhau> _ListWhere(string keyword, SqlConnection conn)
		{
			if (conn.ConnectionString == "")
			{
				return null;
		...
		//Insert ob
		public bool _InsertOb(Ob_DM_ChietKhau ob, SqlConnection conn)
		{
			if (conn.ConnectionString == "")
			{
				return false;
			...
			SqlCommand command = new SqlCommand
			{
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = @"Insert Into DM_ChietKhau (MaChietKhau, TenChietKhau) Values(@MaChietKhau, @TenChietKhau)"
			};
		...
		public bool _DeleteWhere(string condition, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand
			{
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Delete From DM_ChietKhau Where " + condition
			};
			try
			{
				command.ExecuteNonQuery();
		...
		public bool _CheckWhere(string condition, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand
			{
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Select * From DM_ChietKhau Where " + condition
			};
			try
			{
				SqlDataReader reader = command.ExecuteReader();
		...
		public double _Count(string nameColumn, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand {
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Select Count(" + nameColumn + ") From DM_ChietKhau"
			};
			try
			{
				SqlDataReader reader = command.ExecuteReader();
		...
		public double _CountWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand {
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Select Count(" + nameColumn + ") From DM_ChietKhau Where " + keyword +""
			};
			try
			{
				SqlDataReader reader = command.ExecuteReader();
		...
		public double _Min(string nameColumn, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Min(" + nameColumn + ") From DM_ChietKhau"
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _MinWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Min(" + nameColumn + ") From DM_ChietKhau Where " + keyword +""
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _Max(string nameColumn, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select max(" + nameColumn + ") from DM_ChietKhau"
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _MaxWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Max(" + nameColumn + ") From DM_ChietKhau Where " + keyword +""
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _Sum(string nameColumn, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Sum(" + nameColumn + ") From DM_ChietKhau"
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _SumWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Sum(" + nameColumn + ") From DM_ChietKhau Where " + keyword +""
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public string _CreateID(string ColumnName, string Character, SqlConnection conn)
		{
			...
			if (conn.ConnectionString != "")
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select isnull(max(cast(substring(" + ColumnName + ",5,10) as decimal)),0) From DM_ChietKhau"
				};
				SqlDataReader reader = command.ExecuteReader();
				while (reader.Read())
				{
