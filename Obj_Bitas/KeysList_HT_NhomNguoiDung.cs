...
//CreateTimes : 04/04/2016 10:44:45.AM

namespace Bitas
{
	public class KeysList_HT_NhomNguoiDung
	{
		//ListAll
		public List<Ob_HT_NhomNguoiDung> _ListAll(SqlConnection conn)
		{
			...
			while (reader.Read())
			{
				...
				if(reader[1] != DBNull.Value){ ob.MANHOMND = Convert.ToString(reader[1]);}
				if(reader[2] != DBNull.Value){ ob.TENNHOMND = Convert.ToString(reader[2]);}
				...
				if(reader[6] != DBNull.Value){ ob.CREATEBY = Convert.ToString(reader[6]);}
				list.Add(ob);
			}
			command.Dispose();
			command = null;
		...
		//ListAll
		public List<Ob_HT_NhomNguoiDung> _ListAll_sp(string StoredName, SqlConnection conn)
		{
			if (conn.ConnectionString == "")
			{
				return null;
		...
		//ListWhere
		public List<Ob_HT_NhomNguoiDung> _ListWhere(string keyword, SqlConnection conn)
		{
			if (conn.ConnectionString == "")
			{
				return null;
		...
		//Insert ob
		public bool _InsertOb(Ob_HT_NhomNguoiDung ob, SqlConnection conn)
		{
			if (conn.ConnectionString == "")
			{
				return false;
			...
			SqlCommand command = new SqlCommand
			{
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = @"Insert Into HT_NhomNguoiDung (MaNhomND, TenNhomND, ModifiedTime, ModifiedBy, CreatedTime, CreateBy) Values(@MaNhomND, @TenNhomND, @ModifiedTime, @ModifiedBy, @CreatedTime, @CreateBy)"
			};
			if (ob.MANHOMND.Trim() == "")
		...
		public bool _DeleteWhere(string condition, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand
			{
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Delete From HT_NhomNguoiDung Where " + condition
			};
			try
			{
				command.ExecuteNonQuery();
		...
		public bool _CheckWhere(string condition, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand
			{
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Select * From HT_NhomNguoiDung Where " + condition
			};
			try
			{
				SqlDataReader reader = command.ExecuteReader();
		...
		public double _Count(string nameColumn, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand {
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Select Count(" + nameColumn + ") From HT_NhomNguoiDung"
			};
			try
			{
				SqlDataReader reader = command.ExecuteReader();
		...
		public double _CountWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			SqlCommand command = new SqlCommand {
				Connection = conn,
				CommandType = CommandType.Text,
				CommandText = "Select Count(" + nameColumn + ") From HT_NhomNguoiDung Where " + keyword +""
			};
			try
			{
				SqlDataReader reader = command.ExecuteReader();
		...
		public double _Min(string nameColumn, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Min(" + nameColumn + ") From HT_NhomNguoiDung"
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _MinWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Min(" + nameColumn + ") From HT_NhomNguoiDung Where " + keyword +""
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _Max(string nameColumn, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select max(" + nameColumn + ") from HT_NhomNguoiDung"
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _MaxWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Max(" + nameColumn + ") From HT_NhomNguoiDung Where " + keyword +""
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _Sum(string nameColumn, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Sum(" + nameColumn + ") From HT_NhomNguoiDung"
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public double _SumWhere(string nameColumn, string keyword, SqlConnection conn)
		{
			...
			if (this._Count(nameColumn,conn) != 0)
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select Sum(" + nameColumn + ") From HT_NhomNguoiDung Where " + keyword +""
				};
				try
				{
					SqlDataReader reader = command.ExecuteReader();
		...
		public string _CreateID(string ColumnName, string Character, SqlConnection conn)
		{
			...
			if (conn.ConnectionString != "")
			{
				...
				SqlCommand command = new SqlCommand
				{
					Connection = conn,
					CommandType = CommandType.Text,
					CommandText = "Select isnull(max(cast(substring(" + ColumnName + ",5,10) as decimal)),0) From HT_NhomNguoiDung"
				};
				SqlDataReader reader = command.ExecuteReader();
				while (reader.Read())
				{
